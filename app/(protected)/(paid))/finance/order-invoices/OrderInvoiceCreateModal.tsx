...
import CustomerCreateQuickModal from "./CustomerCreateQuickModal";
...
export default function OrderInvoiceCreateModal({
  open,
  onClose,
  customers,
  branches,
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch(
        "/api/finance/order-invoices/create",
        {
          method: "POST",
  ...
  { label: "10%", value: "10" },
                ...
                {form.is_vat && (
                  <FormGroup label="Tiền VAT">
                    <MoneyInput value={form.vat_amount ?? 0} disabled />
                  </FormGroup>
                )}

                <FormGroup label="Tổng tiền">
                  <MoneyInput value={form.total_amount ?? 0} disabled />
                </FormGroup>
              </div>
