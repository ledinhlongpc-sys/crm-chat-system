...
export async function POST(req: Request) {
  try {
    ...
    if (!transaction_id || !files.length) {
      return NextResponse.json(
        { error: "Thiếu transaction_id hoặc files" },
        { status: 400 }
      );
    }
    ...
    const uploadedUrls = await Promise.all(
      files.map(async (file) => {
        try {
          ...
          const key = `${tenant_id}/transactions/${transaction_id}/${Date.now()}-${crypto.randomUUID().slice(0, 8)}.${ext}`;
    ...
    const { error } = await supabase
      .from("system_money_transactions")
      .update({
        proof_images: [...oldImages, ...successUrls],
      })
      .eq("id", transaction_id)
      .eq("tenant_id", tenant_id);
