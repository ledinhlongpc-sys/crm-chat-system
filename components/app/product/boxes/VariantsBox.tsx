...
export default function VariantsBox({
  ...
  const updateUnit = (unitId: string, patch: Partial<UnitConversionRow>) => {
    setUnitRows((prev) =>
                  ...
                  <tbody>
                    {variants.map((v, i) => (
                      <React.Fragment key={v.key}>
                        <TableRow>
                          <TableCell width="32px">{null}</TableCell>
                        ...
                        {unitRows
                          ...
                          .map((u) => (
                            <TableRow key={u.id} className="bg-neutral-50 text-sm">
