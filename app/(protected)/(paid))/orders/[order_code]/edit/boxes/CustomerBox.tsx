...
useEffect(() => {
  ...
  if (value?.id === customerId) return; // tránh gọi lại

  const loadCustomer = async () => {
    try {
      const res = await fetch(
        `/api/customers/search?q=${customerId}&limit=1`
      );
      ...
      const option = mapCustomerToOption(row);
  ...
  const setDefaultAddressLocal = (picked: CustomerAddress) => {
    ...
    onChange({
  ...value,
  selected_address: picked, // 👈 QUAN TRỌNG NHẤT
  ...
  default_address: newDefault,
  selected_address:
    value.selected_address?.id === updated.id
      ? { ...value.selected_address, ...updated }
...
/>
      ...
      <>
        <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
          ...
          <div className="xl:col-span-3 space-y-3">
           ...
           {/* ADDRESS */}
              {(value.selected_address ?? value.default_address) ? (
  ...
  {(value.selected_address ?? value.default_address)?.receiver_name || value.name || "—"}

  {((value.selected_address ?? value.default_address)?.receiver_phone || value.phone) &&
    ` - ${
      (value.selected_address ?? value.default_address)?.receiver_phone || value.phone
    }`}
</div>
                    ...
                    <div className={textUI.body}>
                      {buildAddressLine(value.selected_address ?? value.default_address)}
                    </div>
                  </div>
                </div>
              ) : (
    ...
    <AddAddressCustomerQuickModal
      ...
      onCreated={(createdAddress) => {
        ...
        onChange({
  ...value,
  selected_address: createdAddress, // 👈 THÊM
  ...
  onConfirm={async () => {
    ...
    const res = await fetch(
      `/api/customers/address/delete?id=${deletingAddress.id}`,
      { method: "DELETE" }
    );
    ...
    removeAddressLocal(deletingAddress.id);
