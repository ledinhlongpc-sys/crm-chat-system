...
const getImageUrl = (val?: string) => {
  ...
  return `${process.env.NEXT_PUBLIC_R2_PUBLIC_URL}/${val}`;
};
...
previewUrl: getImageUrl(img.url), // 🔥 FIX
url: getImageUrl(img.url),        // 🔥 FIX
      path: img.path,
      status: "done",
    }))
  );
...
useEffect(() => {
const done = items
.filter((i) => i.status === "done" && i.url && i.path)
.map((i) => ({
url: i.url!,
path: i.path!,
}));
...
return (
<FormBox title={`Ảnh sản phẩm (${items.length}/${MAX_IMAGES})`}> <FormGroup> <div className="flex items-center gap-4">
{items[0] && (
<div
draggable
...
className="relative w-48 h-48 shrink-0 cursor-move rounded-lg overflow-hidden bg-white border-2 border-blue-500"
> <img
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(96px,1fr))] gap-4 flex-1">
        {items.slice(1).map((img, index) => {
          ...
          return (
            <div
              ...
              className="relative w-24 h-24 cursor-move rounded-lg overflow-hidden bg-white border border-neutral-300 group"
            >
              <img
                src={img.previewUrl}
                className="w-full h-full object-contain bg-white"
        ...
        {items.length < MAX_IMAGES && (
          <div
            ...
            className="w-24 h-24 rounded-lg border-2 border-dashed flex items-center justify-center cursor-pointer text-neutral-400 hover:border-blue-500 hover:text-blue-500"
          >
            +
            <input
              ref={inputRef}
